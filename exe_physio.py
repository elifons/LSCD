...
parser = argparse.ArgumentParser(description="Physio")
...
parser.add_argument("--wandb_name", type=str, default="full_physio")
...
foldername = f"./save_ICML/physio_fold{args.nfold}_{args.model}_{args.testmissingratio}/"
...
if args.model in models_dict:
	model = models_dict[args.model](config=config, device=args.device, target_dim=35, timesteps=t, dataset="Physio").to(args.device)
else:
...
if args.modelfolder == "":
    ...
    model.load_state_dict(torch.load("./save_ICML/" + args.modelfolder + "/model.pth"))
